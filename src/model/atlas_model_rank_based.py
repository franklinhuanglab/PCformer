...
class AtlasEncoderRB(nn.Module):
    ...
    def forward(self, embeddings_BLD, key_padding_mask, return_attn: bool = False):
        """
        ...
        attn_out_LBD, attn_weights = self.attention(
            x_LBD, x_LBD, x_LBD, 
            key_padding_mask=key_padding_mask,     # [B,L], True==PAD
            need_weights=True,
            average_attn_weights=False             # <-- keep heads: [B,H,L,L]
        )
        #self.last_attention_weights = attn_weights  # Store for extraction
        
        attn_out_BLD = attn_out_LBD.permute(1, 0, 2)      # Permute back to [B,L,D]

        # Residual connection and normalization
        x = self.norm1(attn_out_BLD + embeddings_BLD)
        x = self.dropout(x)
...
class AtlasModelRankBased(nn.Module):
    ...
    Designed for self-supervised tasks (MLM predictions).
    ...
    def forward(self, input_ids_BL, key_padding_mask, attn_mask=None, return_attn: bool = False):
        """
        ...
        """
        x = self.embeddings(input_ids_BL)
        all_attn = []
        #embeddings_BLD = self.embeddings(input_ids_BL)
        for enc in self.encoders:
            x, aw = enc(x, key_padding_mask, return_attn=return_attn)
            if return_attn: all_attn.append(aw)  # list of [B,H,L,L]
            #embeddings_BLD = encoder(embeddings_BLD,key_padding_mask)
        preds = self.lm_head(x)
        #preds_BLV = self.lm_head(embeddings_BLD)
        
        # return preds_BLV
        return (preds, all_attn) if return_attn else preds
